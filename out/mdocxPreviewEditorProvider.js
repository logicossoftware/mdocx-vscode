...
exports.MdocxPreviewEditorProvider = void 0;
...
class MdocxPreviewEditorProvider {
    _context;
    ...
    async resolveCustomEditor(document, webviewPanel, _token) {
        ...
        const messageDisposable = webviewPanel.webview.onDidReceiveMessage(async (message) => {
            ...
            if (message.type === 'edit') {
                ...
                const editUri = mdocxFileSystemProvider_js_1.MdocxFileSystemProvider.buildUri(document.uri, pathToEdit);
            ...
            if (message.type === 'removeMedia') {
                const confirm = await vscode.window.showWarningMessage(`Remove media "${message.id}" from MDOCX?`, { modal: true }, 'Remove');
                if (confirm === 'Remove') {
    ...
    async renderDocument(resource, selectedPath) {
        try {
            ...
            if (!Array.isArray(doc.markdown.files))
                doc.markdown.files = [];
            doc.media = doc.media || { items: [] };
            if (!Array.isArray(doc.media.items))
                doc.media.items = [];
            const fileList = doc.markdown.files.map((file) => file.path).sort((a, b) => a.localeCompare(b));
            ...
            const rootFile = doc.markdown.files.find((file) => file.path === rootPath) ?? doc.markdown.files[0];
            if (!rootFile) {
                return {
                    type: 'render',
                    path: selectedPath ?? '',
            ...
            const metadata = {
                ...
                root: doc.markdown.rootPath ?? doc.metadata?.root,
            ...
            const mediaItems = (doc.media.items || []).map((item) => {
                ...
                if (item.mimeType?.startsWith('image/') && item.data && item.data.byteLength < 500 * 1024) {
                    info.dataUri = `data:${item.mimeType};base64,${buffer_1.Buffer.from(item.data).toString('base64')}`;
                }
                return info;
            });
            return {
    ...
    async saveMetadata(resource, metadata) {
        ...
        if (metadata.author !== undefined)
            existingMetadata.creator = metadata.author;
        if (metadata.root !== undefined)
            existingMetadata.root = metadata.root;
    ...
    async removeMedia(resource, mediaId) {
        const bytes = await vscode.workspace.fs.readFile(resource);
        const { readMdocx, writeMdocxAsync } = await import('ts-mdocx');
        const doc = await readMdocx(bytes);
        // Defensive normalization
        ...
        if (!Array.isArray(doc.media.items))
            doc.media.items = [];
        const index = doc.media.items.findIndex((item) => item.id === mediaId);
        if (index >= 0) {
    ...
    renderMarkdownToHtml(markdown, options) {
        ...
        const parsed = marked_1.marked.parse(markdown, { renderer, walkTokens });
    ...
    tryResolveMediaHrefToDataUri(resolver, href, fromFile) {
        ...
        const base64 = buffer_1.Buffer.from(item.data).toString('base64');
    ...
    inferMimeType(item) {
        ...
        // SVG (best-effort)
        try {
            const head = new util_1.TextDecoder('utf-8').decode(bytes.slice(0, 256));
...
exports.MdocxPreviewEditorProvider = MdocxPreviewEditorProvider;
//# sourceMappingURL=mdocxPreviewEditorProvider.js.map